...
<div class="wrap">
	...
	<form action="" method="post" id="summarize_posts_form">
		<h3><label for="shortcode_name">group_concat_max_len</label></h3>
		<p>
		...
		<input type="text" id="group_concat_max_len" name="group_concat_max_len" value="<?php print self::$options['group_concat_max_len']; ?>" /> <strong>group_concat_max_len</strong> <em>(number of characters, 4096 recommended)</em>
		</p>

		<h3><label for="output_type">Output Type</label></h3>
		<p>The output type determines how results are returned inside of your theme files via the <code>$wpdb->get_results()</code> function.<br/>
		Object returns results whose attributes must be accessed via arrow notation, e.g. <code>$r->post_title</code>.<br/>
		ARRAY_A returns results whose attributes must be accessed via array notation, e.g. <code>$r['post_title']</code>.</p>
		<p>You can override this setting per query, e.g. 
		<pre>
		$Q = new GetPostsQuery();
		...
		</select> <strong>Output Type</strong>
